...
public class SimpleUI {
  ...
  void setRadioButtonOff(String groupName, String componentType) {
    for (Widget w : widgetList) {
      if ( w.UIComponentType.equals(componentType)) {
        if ( ((RadioButton)w).radioGroupName.equals(groupName))  w.selected = false;
      }
    }
  }
...
{ 
  ...
  public void handleMouseEvent(String mouseEventType, int x, int y)
  {
    // rollover true when cursor over button
    rollover = isInMe(x, y) && (mouseEventType.equals("mouseMoved") || mouseEventType.equals("mousePressed"));

    // mouse released -- button clicked
    if ( isInMe(x, y) && mouseEventType.equals("mouseReleased"))
    {     
      parentManager.setRadioButtonOff(this.radioGroupName, this.UIComponentType);
      selected = true;
...
/////////////////////////////////////////////////////////////////////
//                           ColourSwatch
//  Foreground and background colour swatch selected via colourbox
class ColourSwatch extends RadioButton
{
  color col = color(0);
  int size = 40;

  public ColourSwatch(String uiname, int x, int y, SimpleUI manager, color defaultColour)
  {
    // initialise super, already know this will be in colourswatch radiogroup
    super(uiname, x, y, "", "ColourSwatch", manager);

    radioGroupName = "ColourSwatch";
    UIComponentType = "ColourSwatch";
    parentManager = manager;   
    col = defaultColour;
  ...
  // draw the box and highlight selected
  public void drawMe()
  {
    pushStyle();  
    fill(col);
